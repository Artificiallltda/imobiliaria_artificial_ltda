...
def add_favorite(
    ...
    """Adicionar imóvel aos favoritos do usuário (opcionalmente vinculado a um lead)"""
    ...
    if request.lead_id:
        try:
            ...
            lead_obj = db.query(Leads).filter(Leads.id == lead_uuid).first()
            if not lead_obj:
...
@router.get("/leads/{lead_id}", response_model=List[FavoriteResponse])
def list_lead_favorites(
    lead_id: str,
    current_user: Users = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Listar favoritos de um lead específico com dados dos imóveis"""
    # Validar UUID do lead
    try:
    ...
    # Buscar favoritos do lead
    favorites = (
        db.query(Favorites)
        .filter(
            Favorites.user_id == current_user.id,
            Favorites.lead_id == lead_uuid
        )
        .all()
    )
